...
class OdooConnectApiLine(models.Model):
    ...
    _sql_constraints = [
        ('unique_api', 'UNIQUE(method,model_id, name, api_name)',
        ...
        if method == 'report':
            if self.report_response_type == 'url':
                if self.report_type == 'pdf':
                    ...
                    return "/report/xlsx/%s/%s" % (self.api_name, self.name)
            elif self.report_response_type == 'file':
                print("type", self.report_response_type)
