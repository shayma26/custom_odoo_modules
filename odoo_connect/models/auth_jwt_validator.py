...
class AuthJwtValidator(models.Model):
    ...
    partner_id_strategy = fields.Selection([("email", "From email claim"),("id", "From ID claim")])
    ...
    def _get_partner_id(self, payload):
        ...
        if self.partner_id_strategy == "email":
            ...
            if len(partner) != 1:
                ...
                return
            return partner.id
        elif self.partner_id_strategy == "id":
            partner_id = payload.get("id")
            if not partner_id:
                _logger.debug("JWT payload does not have an ID claim")
                return
            partner = self.env["res.partner"].sudo().search([("id", "=", partner_id)])
            if len(partner) != 1:
                _logger.debug("%d partners found for ID %s", len(partner), partner_id)
                return
            return partner.id
